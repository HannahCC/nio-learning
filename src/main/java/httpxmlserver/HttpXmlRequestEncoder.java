...
public class HttpXmlRequestEncoder extends AbstractHttpXmlEncoder<HttpXmlRequest> {
	...
	protected void encode(ChannelHandlerContext ctx, HttpXmlRequest msg, List<Object> out) throws Exception {
		ByteBuf body = encode0(ctx, msg.getBody());//object to xmlstr, xmlstr to bytebuf
		...
		HttpHeaders.setContentLength(request, body.readableBytes()); //如果request不为null，那么编码后的body什么时候被加入到request？
		out.add(request);
	}
